...
-- SEARCH WORKING CODE FILES
local function search_working_files()
	local fzf_lua = require("fzf-lua")

	fzf_lua.files({
		prompt = "Search Working Files> ",
		cwd = vim.fn.getcwd(),
...
-- SEARCH WORDS INSIDE WORKING CODE
local function search_working_words()
	local fzf_lua = require("fzf-lua")

	fzf_lua.live_grep({
		prompt = "Search Working Words> ",
		cwd = vim.fn.getcwd(),
		winopts = { preview = { hidden = false } },
		file_ignore_patterns = {
			"node_modules/*",
...
-- Keymaps
vim.keymap.set("n", "<leader>rS", search_working_files, { desc = "Search Working Code Files" })
vim.keymap.set("n", "<leader>rW", search_working_words, { desc = "Search Words in Working Code" })
return M
