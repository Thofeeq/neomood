return {
	{
		...
		keys = {
			{
				"<leader>lf",
				"<cmd>lua require('tmux-awesome-manager').list_terms()<CR>",
			...
			{
				"<leader>ll",
				"<cmd>lua require('tmux-awesome-manager').list_open_terms()<CR>",
				desc = "Search Open Terminals",
			},
			{
				"<leader>lk",
				function()
		...
		config = function()
			...
			-- Rails server with custom ENV options
			set("<leader>rS", {
				cmd = "LOCALHOST_SSL=true ENABLE_API=1 ENABLE_HARNESS=1 rails s",
				name = "Rails Server (Custom)",
				open_as = "pane",
				visit_first_call = true,
			})
